...
parser.add_argument("-preprocess_data_file", "--preprocess_data_file", help="the data path of test",default="/Users/yezhenxu/python3_project/tensorflow_project/youtube_8m/youtube-8m_neXtVlad/yt8m_pca/eigenvals.npy")  # 测试集路径



parser.add_argument("-b", "--batch_size", help="the size of batch ", default=3)  # mini-batch的数目
...
parser.add_argument("-cate1_vocab", "--youtu_8m_cate1_dict", help="the data path of tag_cate1_vocab",
                    default="/Users/yezhenxu/python3_project/tensorflow_project/youtube_8m/youtube-8m_neXtVlad/vocab/youtube_cate1.txt")  # tag_cate1_vocab
...
parser.add_argument("-youtube_ground_id_to_cate1_cate2", "--youtube_ground_id_to_cate1_cate2", help="the data path of tag_cate2_vocab",
                    default="/Users/yezhenxu/python3_project/tensorflow_project/youtube_8m/youtube-8m_neXtVlad/vocab/youtube_ground_id_to_cate1_cate2.txt")  # tag_cate2_vocab

parser.add_argument("-train_path", "--train_path", help="the data path of train", default="/Users/yezhenxu/python3_project/tensorflow_project/youtube_8m/my_model/data")  # 训练目录
parser.add_argument("-valid_path", "--valid_path", help="the data path of valid", default="/Users/yezhenxu/python3_project/tensorflow_project/youtube_8m/my_model/data")  # 验证集合，用来挑选保存最好的模型
parser.add_argument("-rf", "--report_freq", help="frequency to report loss", default=1,type=int)  # 打印频率，每隔300个step，即经过300个mini-batch,输出训练的loss
parser.add_argument("-vf", "--valid_freq", help="frequency to do validation", default=1, type=int)  # 验证频率，每隔300个step，即经过300个mini-batch,验证当前验证集合的loss
parser.add_argument("-hd", "--hidden_size", help="the units of hidden", default=128)  # 隐藏层的数目
parser.add_argument("-e", "--epoch", help="the number of epoch", default=200)  # 轮数，即训练集合过几遍模型。
parser.add_argument("-m", "--model_type", help="the model for [train | predict_test | predict_valid]", default="predict_valid")  # 控制开关，即训练or测试
parser.add_argument("-md", "--model_dir", help="the dir of save model", default="./mymodel")  # 保存ckpt模型的路径
parser.add_argument("-pb", "--pb_savePath", help="the dir of save model", default="./mymodel/pb")  # 保存pb模型的路径
parser.add_argument("-test_path", "--test_path", help="the data path of test",default="/Users/yezhenxu/python3_project/tensorflow_project/youtube_8m/my_model/data")  # 测试集路径



parser.add_argument("-predict_out_path", "--predict_out_path", help="the data path of predict_out",default="./testSet.txt")  # 测试文件的预测输出路径
parser.add_argument("-predict_cate1", "--predict_cate1", help="the data path of predict_out",default="./validSet.cate1.txt")  # 测试文件的预测输出路径
parser.add_argument("-predict_cate2", "--predict_cate2", help="the data path of predict_out",default="./validSet.cate2.txt")  # 测试文件的预测输出路径
    ...
    with tf.Session(config=config) as sess:
        ...
        try:
            while True:
                ...
                if model.global_step.eval() > 1 and model.global_step.eval() % args.valid_freq == 0:
                    # 统计验证集的准确率
